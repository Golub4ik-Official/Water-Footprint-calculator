...
// Прокрутка к секции калькулятора (вызывается из HTML кнопки)
...
// ============================================
// ПРЕСЕТЫ СЦЕНАРИЕВ ИСПОЛЬЗОВАНИЯ
// Заранее подготовленные наборы значений для разных типов пользователей
// ============================================
const presets = {
    eco: {
        ...
        values: {
            // Бытовое потребление
            showerTime: 5,      // 5 минут душа в день
            baths: 0,           // 0 ванн в неделю
            dishwashing: 5,     // 5 минут мытья посуды в день
            laundry: 2,         // 2 стирки в неделю
            toiletFlush: 4,     // 4 смыва в день
...
// Управление переходом между шагами формы
// ============================================
let currentStep = 1;
const totalSteps = 3;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Переключатель режима расчёта (быстрый/подробный)
...
// Обновление отображения индикатора прогресса и активной секции
...
// Обновление состояния кнопок навигации (активна/отключена)
...
// Обновление описания текущего режима (быстрый/подробный)
...
// Инициализация синхронизации всех ползунков с числовыми полями
...
// Применение выбранного пресета к полям формы
...
// Определение единицы измерения для поля слайдера
function getUnit(sliderId) {
    if (sliderId === 'shower-time' || sliderId === 'dishwashing') return ' мин';
    if (sliderId === 'baths' || sliderId === 'laundry') return ' раз';
    if (sliderId === 'toilet-flush') return ' раз';
...
// Сброс всех значений формы к значениям по умолчанию
...
// Показ всплывающего уведомления о сбросе
...
// Главная функция расчёта водного следа на основе введённых данных
function calculateWaterFootprint() {
    ...
    // Расчёт: (вес в граммах / 1000) * водный след на кг
    const meatFishWater = 
        (beef / 1000 * 15415) +      // Говядина: 15415 л/кг
        (pork / 1000 * 5988) +       // Свинина: 5988 л/кг
        (lamb / 1000 * 10412) +      // Баранина: 10412 л/кг
        (goatMeat / 1000 * 5521) +   // Козлятина: 5521 л/кг
        (chicken / 1000 * 4325) +    // Курица: 4325 л/кг
        (fish / 1000 * 5000);        // Рыба: 5000 л/кг
    
    const dairyWater = 
        (milk / 1000 * 1020) +       // Молоко: 1020 л/кг
        (cheese / 1000 * 5553) +     // Сыр: 5553 л/кг
        (yogurt / 1000 * 1020) +     // Йогурт: 1020 л/кг
        (butter / 1000 * 5553) +     // Масло: 5553 л/кг
        (cream / 1000 * 1020) +      // Сливки: 1020 л/кг
        (eggs * 50 / 1000 * 3265);   // Яйца: 3265 л/100шт
    
    const grainsWater = 
        (bread / 1000 * 1608) +      // Хлеб: 1608 л/кг
        (rice / 1000 * 1673) +       // Рис: 1673 л/кг
        (wheat / 1000 * 1827) +      // Пшеница: 1827 л/кг
        (oats / 1000 * 1788) +       // Овсянка: 1788 л/кг
    ...
    const beveragesWater = 
        (coffee / 1000 * 130) +          // Кофе: 130 л/кг
        (tea / 1000 * 27) +              // Чай: 27 л/кг
        (juice / 1000 * 1000) +          // Соки: 1000 л/кг
        (softDrinks / 1000 * 300) +      // Газировка: 300 л/кг
        (beer / 1000 * 300) +            // Пиво: 300 л/кг
        (wine / 1000 * 870);             // Вино: 870 л/кг
    
    const oilsWater = 
        (vegetableOil / 1000 * 2575) +   // Растительное масло: 2575 л/кг
        (oliveOil / 1000 * 14431) +      // Оливковое масло: 14431 л/кг
        (mayonnaise / 1000 * 3000);      // Майонез: 3000 л/кг
    
    const legumesNutsWater = 
        (beans / 1000 * 5053) +          // Фасоль: 5053 л/кг
        (lentils / 1000 * 5874) +        // Чечевица: 5874 л/кг
        (peas / 1000 * 1979) +           // Горох: 1979 л/кг
        (nuts / 1000 * 9063) +           // Орехи: 9063 л/кг
        (peanuts / 1000 * 3974);         // Арахис: 3974 л/кг
    
    const sweetsWater = 
        (sugar / 1000 * 197) +           // Сахар: 197 л/кг
        (chocolate / 1000 * 17196) +     // Шоколад: 17196 л/кг
        (honey / 1000 * 1000);           // Мёд: 1000 л/кг
    ...
    const clothesWater = clothes * 2700 / 30;  // 2700 л на одну вещь одежды, делим на дни месяца
    const electronicsWater = electronics * 1600 / 365;  // 1600 л на покупку электроники, делим на дни года
    const booksWater = books * 1000 / 365;  // 1000 л на книгу, делим на дни года
    const toiletriesWater = toiletries * 200 / 30;  // 200 л на средства гигиены, делим на дни месяца
...
// Генерация текста сравнения с средними показателями
...
// Эффект параллакса для фона главной секции
...
// Инициализация переключения таблиц с данными
document.addEventListener('DOMContentLoaded', function() {
